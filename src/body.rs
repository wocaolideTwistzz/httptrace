        ...
        match self.inner {
            ...
            Inner::Streaming(ref mut body) => Poll::Ready(
                ready!(Pin::new(body).poll_frame(cx))
                    .map(|opt_chunk| opt_chunk.map_err(Into::into)),
            ),
        }
    }
    ...
    B::Error: Into<Box<dyn std::error::Error + Send + Sync>>,
{
    ...
    fn poll_frame(
        ...
        if let Poll::Ready(()) = this.timeout.as_mut().poll(cx) {
            return Poll::Ready(Some(Err(crate::Error::BodyTimeout)));
        }
        Poll::Ready(
            ready!(this.inner.poll_frame(cx))
                .map(|opt_chunk| opt_chunk.map_err(|e| e.into().into())),
        )
    }
...
impl<B> HttpBody for ReadTimeoutBody<B>
where
    B: HttpBody,
    B::Error: Into<Box<dyn std::error::Error + Send + Sync>>,
{
